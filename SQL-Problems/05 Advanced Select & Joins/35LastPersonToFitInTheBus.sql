...
SELECT
    ...
    turn,
    weight,
    @total := @total + weight AS running_total
FROM
    Queue, (SELECT @total := 0) AS init
ORDER BY Turn ASC
)
-- Finally, we select the last person who made it on board by ordering in descending turn and limiting to 1
SELECT
    person_name
FROM
    cte
